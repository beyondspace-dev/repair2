...
function genEl(element: Types.Element, registerUnsubscriber: (id: string, cb: () => void) => void) {
  if (element.type === "empty") {
    ...
    if (element.payload.content)
      el[element.payload.isHtml ? "innerHTML" : "textContent"] = String(element.payload.content);
...
export default class RepairElement extends HTMLElement {
  ...
  setListeners() {
    ...
    const repeating: Map<Types.Listener, { count: number; timeout?: NodeJS.Timeout }> = new Map();
    ...
    const activeListener = (listener: Types.Listener) => {
      ...
      if (listener.repeatInterval)
        repeatInfo.timeout = setTimeout(() => repeating.delete(listener), listener.repeatInterval);
    };
  ...
  getStyleString(absolute: boolean = this.element.absolute, pos: Types.Coord = this.element.pos) {
    ...
    return (
      (absolute ? `position: absolute;${coordToStyleString(pos)}` : "") + (this.element.style ?? "")
    );
  }
  get styleString() {
