...
export class PluginManager {
  ...
  private async changePluginInfo(
    ...
    if (newInfo.name !== oldInfo.name) {
      ...
      if (oldInfo.linked) {
        const replaceSucceed = await this.pluginLinkService.replaceName(oldInfo.name, newInfo.name);
  ...
  reportPluginError(
    ...
    const log = this.pluginDiagnostics[pluginPhase === "build" ? "buildFailed" : "runtimeError"](
      { name, type },
      [title, error],
      phase ?? pluginPhase,
      logType,
      from ??
        (typeof error !== "string" && "fileName" in error && typeof error.fileName === "string"
          ? {
    ...
    if (
      !log ||
      (prevError &&
        (!log.phase || getPluginPhasePriority(prevError.phase) < getPluginPhasePriority(log.phase)))
    )
      return;
    plugin.error[target] = {
